...
function renderAnswerExtensions(question){
  const render=(className,label,title,html='',paragraphs=[])=>html||paragraphs.length?`<details class="answer-extension ${className}"><summary><span><small>${label}</small><b>${title}</b></span><i aria-hidden="true"></i></summary><div class="${html?'practice-rich':''}">${html||paragraphs.map(paragraph=>`<p>${escapeHtml(paragraph)}</p>`).join('')}</div></details>`:''
  const content=render('analysis-extension','思路拆解','问题分析',question.problemAnalysisHtml,question.problemAnalysis)+render('pitfalls-extension','容易答偏','踩坑误区',question.pitfallsHtml,question.pitfalls)
  return content?`<section class="answer-extensions" aria-label="答题补充">${content}</section>`:''
}
function renderPracticeQuestion(question){
  ...
  const answerContent=question.answerHtml?`<div class="practice-rich answer-rich">${question.answerHtml}</div>`:`<div class="answer-copy">${paragraphs}</div>`
...
const practiceDescription='随机练习贴近项目、生产故障和系统设计的 Java 高频面试题，先看简短总结，再用一到两分钟自然回答并继续追问。'
